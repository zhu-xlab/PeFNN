...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_kernel_size=1, input_stride=1, input_padding=0):
        ...
        self.Wh1_u = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh2_u = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=0, bias=True, )
        self.Wh3_u = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=0, bias=True, )
        # Aggregate conv layer 1x1
        self.Wh4_u = nn.Conv2d(in_channels=8, out_channels=1, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, h):
        ...
        u_res = self.W_laplace(u_pad) + self.Wh4_u(self.Wh1_u(h) * self.Wh2_u(h) * self.Wh3_u(h))
...
class RCNN(nn.Module):
    ...
    def __init__(self, input_kernel_size, ini_state, input_stride, input_padding, step=1, effective_step=[1]):
        ...
        complex_ct = sum(par.numel() * (1 + par.is_complex()) for par in cell.parameters())
        real_ct = sum(par.numel() for par in cell.parameters())
        self.cell_complex_ct = complex_ct
        self.cell_real_ct = real_ct
...
def postProcess(output, true, axis_lim, u_lim, v_lim, num, fig_save_path):
    ...
    #
    cf = ax[0, 0].scatter(x_star, y_star, c=u_pred, alpha=0.99, edgecolors='none', cmap='hot', marker='s', s=2.5,
    ...
    #
    cf = ax[0, 1].scatter(x_star, y_star, c=u_star, alpha=0.99, edgecolors='none', cmap='hot', marker='s', s=2.5,
                          vmin=u_min, vmax=u_max)
    ax[0, 1].axis('square')
    ...
    #
    cf = ax[1, 0].scatter(x_star, y_star, c=v_pred, alpha=0.99, edgecolors='none', cmap='hot', marker='s', s=2.5,
    ...
    #
    cf = ax[1, 1].scatter(x_star, y_star, c=v_star, alpha=0.99, edgecolors='none', cmap='hot', marker='s', s=2.5,
                          vmin=v_min, vmax=v_max)
    ax[1, 1].axis('square')
