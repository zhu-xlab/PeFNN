...
parser.add_argument("--results_path", type=str, default="/PATH/results/NS/", help="path to store results")
parser.add_argument("--suffix", type=str, default="seed1", help="suffix to add to the results path")
parser.add_argument("--txt_suffix", type=str, default="ns_V1e-4_N10000_T30.mat_PeFNN_seed1", help="suffix to add to the results txt")
parser.add_argument("--data_path", type=str, default='/PATH/NS/ns_V1e-4_N10000_T30.mat', help="path to the data")
parser.add_argument("--super_path", type=str, default='/PATH/NS/ns_data_V1e-4_N20_T50_R256test.mat', help="path to the superresolution data")
parser.add_argument("--super", type=str, default='True', help="enable superres testing")
...
swe = TRAIN_PATH.split(os.path.sep)[-1][:6] == "2D_rdb"
grid_type = "symmetric"
if args.grid:
    grid_type = args.grid
...
if swe:
    assert T == 24, "T should be 24 for swe"
    T_in = 1
    S = Sx = Sy = 128
    S_super = 128
    T_super = 96
...
if time and not args.time_pad:
    time_modes = 6 # 6 is based on T=10
elif time and swe:
    time_modes = 8
...
if args.model_type in ["FNO2d"]:
    ...
    model = PeFNN(num_channels=num_channels, initial_step=initial_step, modes=modes, width=width,
                  grid_type=grid_type).cuda()
...
if extension == "mat": # incompressible NS
...
elif swe: # shallow water equations
    ...
    with h5py.File(TRAIN_PATH, 'r') as f:
        ...
        data = torch.tensor(data[..., :, 0]).permute(0, 3, 1, 2).permute(0, 2, 3, 1).unsqueeze(-1).numpy()
else:
...
print(f"{args.model_type} done training; \nTest: {test_l2 / ntest}, Super Space Test: {test_space_l2}, Super Time Test: {test_time_l2}")
